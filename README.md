...
| `AI_BROWSER` | Copilot + hunter Obscura browser (`true`/`false`) | `true` |
...
The copilot is on in the Compose stack. It talks to the LiteLLM proxy (`GLM-5.3-Flash` by default), can read recon data, enqueue existing scan modules, send HTTP to every host that is already a Reconner target or asset, run a **container shell** (`exec`) for local toolchain and in-scope CLIs, and drive **Obscura** (`browser_open` / snapshot / click / fill) for JS-rendered pages. The 24/7 hunter gets the same `exec` and Obscura tools; hunter navigations and host-mentioning shell commands share the per-host HTTP budget. Hosts are scope-checked the same way as `http_request`; cloud metadata is blocked; `/data` is off limits. Operator-confirmed hunter leads enqueue a URL-scoped verify; confirmed findings still only come from Reconner’s verification engines.
